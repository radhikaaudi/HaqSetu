...
test("widow profile is deterministically eligible", () => {
  const profile: CitizenProfile = { marital_status: voice("widow"), annual_income_inr: voice(120000), state: voice("Uttar Pradesh"), raw_documents: [] };
  const verdict = evaluateScheme(profile, scheme);
  assert.equal(verdict.status, "eligible");
  assert.deepEqual(verdict.matched_rules.map((rule) => rule.id), ["must_be_widow", "income_below_limit", "resident"]);
...
test("profile with an unknown eligibility fact needs more information", () => {
  ...
  assert.deepEqual(verdict.matched_rules.map((rule) => rule.id), ["must_be_widow", "resident"]);
});
