...
const fakeLlm: EntitlementLlmClient = {
  async parse<T>(_input: string, _instructions: string, _schema: z.ZodType<T>, _schemaName: string) {
    // The agent's only model call produces localized explanations; eligibility is code-decided.
    return { explanations: [] } as T;
  }
};
...
test("concurrent scheme engine keeps eligibility deterministic and ranks eligible schemes first", async () => {
  const profile: CitizenProfile = {
    marital_status: voice("widow"), age: voice(41), annual_income_inr: voice(50000), state: voice("Uttar Pradesh"), household_size: voice(3),
  ...
  // At 41 she is definitively too young for the old-age pension (needs 60+) — a code-decided rejection.
  assert.equal(verdicts.find((verdict) => verdict.scheme_id === "old_age_pension")?.status, "not_eligible");
});
